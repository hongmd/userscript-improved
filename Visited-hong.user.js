...
(function () {
  ...
  const css_a_visited =
    " a:visited, a:visited * { color: %COLOR% !important; } ";
  ...
  function isExceptSite(except, site) {
    ...
    for (var i = 0; i < exceptList.length; i++) {
      var str = exceptList[i].replace(/\s/gi, "");

      var str1 = str;
      if (str1.indexOf(".") != 0 && str1.indexOf("/") != 0) str1 = "." + str1;

      var str2 = str;
      if (str2.indexOf("://") != 0) str2 = "://" + str2;

      if (str != "" && (site.indexOf(str1) > -1 || site.indexOf(str2) > -1)) {
        return true;
      }
    }
    return false;
  ...
  function assignColor(css, color) {
    return css.replace(/%COLOR%/gi, color);
  }
